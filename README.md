...
| `chat-service` | диалоги teacher-student, сообщения, вложения, read markers | `chat_db_shard0/1` |
...
- `repositories/` — роутинг по `dialog_id` в `chat_db_shard0/1`;
...
Chat-service использует `CHAT_DATABASE_URL_SHARD0` и
`CHAT_DATABASE_URL_SHARD1`. Для совместимого одно-БД режима обе переменные
можно направить в одну физическую БД; список диалогов удаляет дубли
scatter-gather. Решение и ограничения описаны в `docs/adr/0002-chat-db-sharding.md`.
