...
Не реализуй зависящий шаг, пока не готов или не замокан его источник. Для новых
асинхронных flows сначала фиксируй event contract/outbox/inbox и только потом
подключай consumer.
...
3. **Границы БД.** Сервис работает только со своей БД. Запрещено: подключаться к
   ...
   данные — только через согласованный сервисный API (gRPC; HTTP только для file multipart).
4. Связь teacher↔student проверяется ТОЛЬКО через identity
   `IdentityService.CheckTeacherStudentAccess` по gRPC.
...
7. Charge создаётся ТОЛЬКО finance consumer-ом из события `lesson.completed`;
   lesson-service пишет событие в outbox, gateway charge не создаёт.
...
Нужен чужой сервис/consumer, которого ещё нет, — НЕ лезть в его код. Завести
client-interface или event contract + временную stub/mock-реализацию у себя
(по контракту). После готовности чужого сервиса — переключить на реальный client
или consumer. Так задача не блокируется чужим кодом.
...
Already implemented in the current architecture:
...
* Internal synchronous calls use gRPC; Kafka is for facts that already happened,
  not for user commands or access checks.
