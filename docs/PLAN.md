...
Статус: **foundation + 5 сервисов в `main`; следующий шаг — api-gateway.**
...
| Старт | foundation в `main` собран; сервисы реализованы, остался api-gateway (см. §14, roadmap) |
...
gateway     -> все                 (проксирование + auth)
```
...
finance     -> file
```
`finance -> lesson` намеренно отсутствует: charge пушит lesson, finance занятие
не тянет. Файлы в assignment/finance хранятся как `file_id` без вызова file.
...
Текущий статус: identity, file, lesson, assignment, finance — реализованы и в
`main`. **Следующая задача — api-gateway.** Полный список оставшихся работ и их
порядок — в `docs/roadmap.md`.
...
```text
lesson.completed          ✅ ВНЕДРЕНО (outbox в lesson -> Kafka -> finance charge; прямой REST убран)
payment_receipt_uploaded  ⬜ 5F
payment_confirmed         ⬜ 5F
payment_rejected          ⬜ 5F
charge_created            ⬜ 5F
assignment_created        ⬜ 5F
submission_uploaded       ⬜ 5F
assignment_reviewed       ⬜ 5F
```

Не делаем сейчас: остальные доменные события (5F), сервисы-консьюмеры
notification/report/chat (5G), Redis, Telegram, Google Calendar, реальные платежи,
родительский аккаунт, повторяющиеся занятия. (Kafka-фундамент и gRPC уже внедрены.) Комментарии к ДЗ в MVP — внутри assignment-service
(это не chat-service).
