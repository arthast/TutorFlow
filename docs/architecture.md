...
учёт, in-app уведомления, дашборды и личный чат.
...
Снаружи открыт только `api-gateway` (8080). Остальное внутреннее.
...
Инфраструктура: PostgreSQL, Kafka (KRaft) + kafka-ui, MinIO (S3), one-shot
...
1. Внешние запросы — только через gateway; внутренние сервисы наружу не торчат.
...
Production hardening (reverse proxy, CI/CD, метрики, Kafka lag/DLQ, бэкапы), realtime-чат
(WebSocket/SSE + Redis), внешняя доставка уведомлений (email/Telegram/push), полноценный
auth, реальные платежи. Очередь работ — `docs/roadmap.md`.
