...
7. **Backend gaps from frontend-plan**:
   - подтвердить/зафиксировать в finance contract, что `receipt.comment`
     заполняется при reject;
   - решить, нужен ли endpoint bulk mark-read уведомлений (`POST /notifications/read-all`).
...
**6.4 Причина отклонения чека в ответе — P2. Уточнить/возможно контракт.**
Фронт читает `receipt.comment` при `rejected` с fallback. Проверить finance: сохраняется ли
`comment`/reason при reject и отдаётся ли в `GET payment-receipts`. Если поле есть, но не
заполняется — фикс ВНУТРИ finance (без контракта). Если поля нет — добавить в finance
proto/OpenAPI. DoD: ученик видит реальную причину отклонения.
