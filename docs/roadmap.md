...
5. **Readiness checks**: развести lightweight `/health` и readiness-зависимости
   (Postgres/Kafka/Redis/MinIO/critical clients) для app-сервисов.
...
### 8.1 Outbox: корректность при нескольких репликах  [libs/events]  ⬜
...
- DoD: сборка + существующие тесты зелёные; при 2 репликах notification в логах
  публикует только один инстанс (второй молчит), дублей `notification.created` нет.

### 8.2 Readiness checks  [все сервисы]  ⬜  (закрывает п.5 «Актуального плана»)
