...
function messageFor(method: string, path: string, code: string): string {
  const context = [
    {
      method: "POST",
  ...
  ].find((item) => item.method === method && item.code === code && matchesPath(item.path, path));

  return context?.message ?? RU_BY_CODE[code] ?? FALLBACK_ERROR_MESSAGE;
}
...
export class ApiError extends Error {
  ...
  constructor(status: number, code: string, raw: string, method = "", path = "") {
    super(messageFor(method, path, code));
    this.code = code;
    this.status = status;
    this.raw = raw;
